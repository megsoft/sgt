    ...
    $.addFlex = function (t, p) {
        ...
        var g = {
            ...
            addData: function (data) { //parse data
                ...
                $('.pReload', this.sDiv).removeClass('loading');
                this.loading = false;
            ...
            populate: function () { //get latest data
                ...
                $('.pReload', this.sDiv).addClass('loading');
            ...
            doSearch: function () {
                ...
                p.qtype = $('select[name=qtype]', g.sDiv).val(); p.query = $('input[name=q]', g.sDiv).val();
                p.qtype = $('select[name=qtype]', g.sDiv).val();
                debugger;
                if (p.query.length >= 11 && p.qtype == "MAWB#") {
                    var qry = p.query.replace(/["~!@#$%^&*\(\)_+=`{}\[\]\|\\:;'<>,.\/?"\- \t\r\n]+/g, '');
                    var SlNo = qry.substring(3, 10);
                    var CheckDigit = qry.substring(10, 11);
                    if (SlNo - (parseInt(SlNo / 7) * 7) != CheckDigit || qry.length != 11) {
                        $("#msg").html("MAWB# " + qry + "   is invalid! (Check digit validation)");
        ...
        if (p.usepager) {
            ...
            if (p.searchitems) {
                ...
                $(g.sDiv).append("<div class='sDiv2'><div class='pReload pButton'><span></span></div><div class='btnseparator'></div><span style='margin-left: 5px;margin-right: 5px;'>Quick Search <input type='text' size='30' name='q' class='qsbox' /><select name='qtype'>" + sopt + "</select></span> <input style='height:24px;display:none;' type='button' value='Search' /><span style='position:absolute;'><img style='cursor:pointer;height: 22px;margin-left: 2px;' class='clearsearch' src='css/images/Eraser.png'><div class='btnseparator'></div></span></div>");
                $('input[value=Search]', g.sDiv).click(function () {
                    g.doSearch();
                });
                $('.pReload', g.sDiv).click(function () { g.populate() });
                $('input[name=q],select[name=qtype]', g.sDiv).keydown(function (e) { if (e.keyCode == 13) { e.preventDefault(); g.doSearch() } });
                $('.clearsearch', g.sDiv).click(function () { $('input[name=q]', g.sDiv).val(''); p.query = ''; g.doSearch(); });
                $(g.bDiv).after(g.sDiv);
                //alert(p.query);
