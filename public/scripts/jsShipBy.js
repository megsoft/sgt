...
$(document).ready(function () {
    ...
    $("[id^=gridedit_]").live("click", function () {
        ...
        $("#myModalLabel").html('Edit Ship by');
        $('.YellowWarning').hide();
        $('.Warning').hide();
        var sourceCtl = $(this);
...
function validate() {
    ...
    if ($("#txtThrough").val().trim() == "") {
        $(".YellowWarning").html("Please Enter Through ");
        $(".YellowWarning").show();
        $("#txtThrough").focus();
...
function Save() {
    ...
    if (validate()) {
        ...
        var xhr;
        var jsondataResource = JSON.stringify({ action: 'save', Id: $("#Id").html().trim(), Through: $("#txtThrough").val().trim()});
        xhr = $.ajax({
            type: "POST",
            url: "../ShipBy/save",      
            data: jsondataResource ,
            ...
            success: function (msg, textStatus, xmlHttp) {
                ...
                if (msg.status == "saved") {
                    ...
                    $("#mmsg").css("color", "green");
                    $("#MessageModal").modal("show");
                    $("#fgrdData").flexReload({ url: "../ShipBy" });
                    result = msg;
                    cancelAdd();
                }
                else if (msg.status == "ER_DUP_ENTRY") {
                    $("#ptitle").html("Warning");
                    $("#mmsg").html(" Record Name  : " + $("#txtThrough").val() + " Already exists ");
                    $("#mmsg").css("color", "red");
                    $("#MessageModal").modal("show");
                    $("#txtThrough").focus();
                }
...
function Edit() {
    ...
    if (validate()) {
        var xhr;
        var jsondataResource = JSON.stringify({ action: 'edit', Through: $("#txtThrough").val(),Id: $("#Id").html() });
        xhr = $.ajax({
            type: "POST",
            url: "../ShipBy/transact",      
            data: jsondataResource ,
