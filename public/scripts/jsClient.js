        ...
        $.ajax({
            ...
            success: function (output, status) {
                ...
                if (item.Id == SelectedID) {
                    ...
                    $("#txtComLegalName").val(item.CompanyLegalName);
                    $("#txtComDBA").val(item.CompanyDBA);
                    $("#txtComFax").val(item.CompanyFax);
                    $("#txtComPhone").val(item.CompanyPhone);       
...
function validate() {
        ...
        if ($("#txtWebsite").val() != "") {
            ...
            if (!re.test(url)) {
                $(".YellowWarning").html("Enter Valid Website url ");
                $(".YellowWarning").show();
                $("#txtWebsite").val('');
                $("#txtWebsite").focus();
            ...
            xhr = $.ajax({
                ...
                success: function (msg, textStatus, xmlHttp) {
                    ...
                    if (msg.status == "saved") {
                        ...
                        $("#MessageModal").modal("show");
                        $("#fgrdData").flexReload({ url: "../Client" });
                        result = msg;
                        cancelAdd();
                    }
                    else if (msg.status == "ER_DUP_ENTRY") {
                        $("#ptitle").html("Warning");
                        $("#mmsg").html(" Client  : " + $("#txtFirstName").val() + " Already exists ");
                        $("#mmsg").css("color", "red");
                        $("#MessageModal").modal("show");
                        $("#txtFirstName").focus();
                    }
...
function SendInvite() {
    ...
    function addclicked(com, grid) {
        $("#myModalLabel").html('Client Creation');
        $("#txtFirstName").val('');
        $("#txtInitial").val('');
        $("#txtLastName").val('');
        $("#txtAddress").val('');
