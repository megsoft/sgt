...
$(document).ready(function () {
    ...
    $("[id^=gridedit_]").live("click", function () {
        ...
        $("#myModalLabel").html('Edit Country');
        $('.YellowWarning').hide();
        $('.Warning').hide();
        var sourceCtl = $(this);
...
function validate() {
    ...
    if ($("#txtCountryName").val().trim() == "") {
        $(".YellowWarning").html("Enter Valid Country Name ");
        $(".YellowWarning").show();
...
function Save() {
    ...
    if (validate()) {
        ...
        var xhr;
        var jsondataResource = JSON.stringify({ action: 'save', Id: $("#Id").html().trim(), CountryName: $("#txtCountryName").val().trim(), Description: $("#txtDescription").val() });
        xhr = $.ajax({
            type: "POST",
            url: "../Country/save",      
            data: jsondataResource ,
            ...
            success: function (msg, textStatus, xmlHttp) {
                ...
                if (msg.status == "saved") {
                    ...
                    $("#mmsg").css("color", "green");
                    $("#MessageModal").modal("show");
                    $("#fgrdData").flexReload({ url: "../Country" });
                    result = msg;
                    cancelAdd();
                }
                else if (msg.status == "ER_DUP_ENTRY") {
                    $("#ptitle").html("Warning");
                    $("#mmsg").html(" Country  : " + $("#txtCountryName").val() + " Already exists ");
                    $("#mmsg").css("color", "red");
                    $("#MessageModal").modal("show");
                    $("#txtCountryName").focus();
                }
...
function Edit() {
    ...
    if (validate()) {
        var xhr;
        var jsondataResource = JSON.stringify({ action: 'edit', CountryName: $("#txtCountryName").val(), Description: $("#txtDescription").val(), Id: $("#Id").html() });
        xhr = $.ajax({
            type: "POST",
            url: "../Country/transact",      
            data: jsondataResource ,
            contentType: 'application/json', // content type sent to server
            dataType: 'json', // Expected data format from server
            processdata: true, // True or False
