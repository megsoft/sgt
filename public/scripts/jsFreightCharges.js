...
$(document).ready(function () {
    ...
    $("[id^=itemedit_]").live("click", function () {
        ...
        $("#myModalLabel").html('Edit Freight Charges');
        $('.YellowWarning').hide();
        $('.Warning').hide();
        var sourceCtl = $(this);
    ...
    function validate() {
        ...
        if ($("#txtType").val() == "") {
            $(".YellowWarning").html("Enter Valid Type ");
            $(".YellowWarning").show();
            $("#txtType").focus();
            ...
            xhr = $.ajax({
                ...
                success: function (msg, textStatus, xmlHttp) {
                    ...
                    if (msg.status == "saved") {
                        ...
                        $("#mmsg").css("color", "green");
                        $("#MessageModal").modal("show");
                        $("#fgrdData").flexReload({ url: "../FreightCharges" });
                        result = msg;
                        cancelAdd();
                    }
                    else if (msg.status == "ER_DUP_ENTRY") {
                        $("#ptitle").html("Warning");
                        $("#mmsg").html(" FreightCharges  : " + $("#txtType").val() + " Already exists ");
                        $("#mmsg").css("color", "red");
                        $("#MessageModal").modal("show");
                        $("#txtType").focus();
                    }
    ...
    function addclicked(com, grid) {
        $("#myModalLabel").html('Add Freight Charges');
        $("#txtType").val('');
        $("#Id").html('');      
        $('#EntryModal').modal('show');
        $(".YellowWarning").hide();
