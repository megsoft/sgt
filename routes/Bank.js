...
exports.BankList = function (req, result) {
   ...
   var mysql = require('mysql');
    ...
    async.series([
      ...
      connection.query("CALL GetBank('tbl.Id  >  0 ','',0,10);", function (err, rows) {
        ...
        if (!err) {
         var page = 1;
           ...
           for (j = 0; j < eval(rows[0]).length; j++) {
                ...
                flexiformat += ' {       "id":"' + row.Id + '", "cell":["' + row.Id + '", "<b><a  id=gridedit_' + row.Id + ' >' + row.BankLegalName + '</a></b>" ,"' + row.CompanyDBA + '","' + row.Address + '","' + row.SwiftCode + '","' + row.CountryName + '","' + row.User + '", "<b><a  id=griddelete_' + row.Id + ' >' + 'Delete' + '</a></b>"  ]    } ,';
            }
    ...
    result.render('PageBank', {
        page_title: "Bank - Node.js",
        data: flexiformat
    });
            }
            else {
...
exports.GetBankData = function (req, result) {
    var flexiformat = '';
    ...
    var rp = req.body.rp;
    connection.query("CALL GetBank('" + wherecond + "','" + sort + "'," + start + "," + rp + ");", function (err, rows) {
...
exports.save = function (req, res) {
    ...
    connection.query(AgentIdSelect, function (err, row, fields) {
        ...
        if (!err) {
            ...
            query = "insert into tblBank  (BankLegalName,CompanyDBA,Address,SwiftCode,CountryId,RefLedgerId) values ('" + req.body.BankLegalName + "', '" + req.body.CompanyDBA + "','" + req.body.Address + "','" + req.body.SwiftCode + "','" + req.body.CountryId + "','" + AgentId + "')";
...
/* Edit,Delete ,get single record (by Id from Bank table ) process */
exports.transact = function (req, result) {
    ...
    if (req.body.Id > 0 && req.body.action == "edit") {
        ...
        connection.query(AgentIdSelect, function (err, row, fields) {
            ...
            if (!err) {
                ...
                query = "update tblBank set BankLegalName='" + req.body.BankLegalName + "',CompanyDBA='" + req.body.CompanyDBA + "',Address='" + req.body.Address + "',SwiftCode='" + req.body.SwiftCode + "',CountryId='" + req.body.CountryId + "',RefLedgerId='" + AgentId + "' where id='" + req.body.Id + "'";
                connection.query(query, function (err, rows, fields) {
    ...
    /* delete code */
        ...
        query = "delete FROM tblBank WHERE id=" + req.body.Id;
        connection.query(query, function (err, rows, fields) {
    ...
    /* Get Record by Id */
          ...
          var i = 1;
          var jsondata = '[';
        query = "SELECT * FROM tblBank WHERE Id=" + req.body.Id;
        connection.query(query, function (err, rows, fields) {
