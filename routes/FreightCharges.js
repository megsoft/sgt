/*
 * GET FreightCharges listing.
 */
var cnt = 0;
var totalrows = 0;
var connection = require('./db');
//var logger = require('./logger');
exports.GetFreightChargeslist = function (req, result) {
    ...
    result.render('PageFreightCharges', {
        page_title: "Freight Charges - Node.js",
        data: flexiformat
    });
        }
        else {
...
exports.GetFreightChargesData = function (req, result) {
    var flexiformat = '';
    ...
    connection.query("CALL GetFreightCharges('" + wherecond + "','" + sort + "'," + start + "," + rp + ");", function (err, rows) {
        if (!err) {
...
exports.save = function (req, res) {
    ...
    var Query = "insert into tblFreightCharges  (Type) values ('" + req.body.Type+ "')";
...
exports.transact = function (req, result) {
    ...
    if (req.body.Id > 0 && req.body.action == "edit") {
        ...
        var Query = "update tblFreightCharges set Type='" + req.body.Type + "'  where Id=" + req.body.Id + "";
        connection.query(Query, function (err, rows, fields) {
            if (!err) {
    ...
    /* delete code */
        ...
        var query = "delete FROM tblFreightCharges WHERE Id=" + req.body.Id;
        connection.query(query, function (err, rows, fields) {
            if (!err) {
                var jsonResponse = JSON.stringify({
                    status: 'deleted'
    ...
    /* Get Record by Id */
        ...
        var i = 1;
        var jsondata = '[';
        var query = "SELECT * FROM tblFreightCharges WHERE id=" + req.body.Id;
        connection.query(query, function (err, rows, fields) {
            if (!err) {
