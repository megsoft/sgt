...
exports.GetInsurancelist = function (req, result) {
    ...
    db.query("CALL GetInsurance('tbl.Id  <> 0 ','',0,10);", function (err, rows) {
    ...
    result.render('PageInsurance', {
        page_title: "Insurance - Node.js",
        data: flexiformat
    });
}
            else {
...
exports.GetInsuranceData = function (req, result) {
    var flexiformat = '';
    ...
    db.query("CALL GetInsurance('" + wherecond + "','" + sort + "'," + start + "," + rp + ");", function (err, rows) {
        if (!err) {
...
exports.save = function (req, res) {
    ...
    var Query = "insert into tblInsurance  (Type) values ('" + req.body.Type+ "')";
...
exports.transact = function (req, result) {
    ...
    if (req.body.Id > 0 && req.body.action == "edit") {
        ...
        var Query = "update tblInsurance set Type='" + req.body.Type + "'  where Id=" + req.body.Id + "";
        db.query(Query, function (err, rows, fields) {
    ...
    /* delete code */
        ...
        var query = "delete FROM tblInsurance WHERE Id=" + req.body.Id;
        db.query(query, function (err, rows, fields) {
            if (!err) {
                var jsonResponse = JSON.stringify({
                    status: 'deleted'
    ...
    /* Get Record by Id */
        ...
        var i = 1;
        var jsondata = '[';
        var query = "SELECT * FROM tblInsurance WHERE id=" + req.body.Id;
        db.query(query, function (err, rows, fields) {
            if (!err) {
