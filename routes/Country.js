...
exports.CountryList = function (req, result) {
    ...
    async.series([
        ...
        function (callback) {
            ...
            if (existsCount > 0) {
                ...
                connection.query("CALL GetCountry('id  <> 0 ','',0,10);", function (err, rows) {
                    if (!err) {
                        ...
                        for (j = 0; j < eval(rows[0]).length; j++) {
                            ...
                            flexiformat += ' {       "id":"' + row.Id + '", "cell":["' + row.Id + '", "<b><a  id=gridedit_' + row.Id + ' >' + row.CountryName + '</a></b>" ,"' + row.Description + '", "<b><a  id=griddelete_' + row.Id + ' >' + 'Delete' + '</a></b>"  ]    } ,';
                        }
                ...
                result.render('PageCountry', {
                    page_title: "Country - Node.js",
                    data: flexiformat
                });
            }
            else {
...
exports.GetCountryData = function (req, result) {
    ...
    connection.query("CALL GetCountry('" + wherecond + "','" + sort + "'," + start + "," + rp + ");", function (err, rows) {
        if (!err) {
...
exports.save = function (req, res) {
    ...
    query = "insert into tblCountry  (CountryName,Description) values ('" + req.body.CountryName + "','" + req.body.Description + "')";
...
exports.transact = function (req, result) {
    ...
    if (req.body.Id > 0 && req.body.action == "edit") {
        ...
        query = "update tblCountry set CountryName='" + req.body.CountryName + "',Description='" + req.body.Description + "' where id='" + req.body.Id + "'";
        connection.query(query, function (err, rows, fields) {
            if (!err) {
                console.log(rows);
    ...
    /* delete code */
        ...
        query = "delete FROM tblCountry WHERE id=" + req.body.Id;
        connection.query(query, function (err, rows, fields) {
            if (!err) {
                var jsonResponse = JSON.stringify({
                    status: 'deleted'
    ...
    /* Get Record by Id */
        ...
        var i = 1;
        var jsondata = '[';
        query = "SELECT * FROM tblCountry WHERE id=" + req.body.Id
        connection.query(query, function (err, rows, fields) {
            if (!err) {
